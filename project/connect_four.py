...
class Connect_Four():
    ...
    def has_winner(self, board_state, winning_length):
        ...
        for d in range(diagonals_start, diagonals_end):
            winner = self.has_winning_line(
                (board_state[i][board_height - i - d - 1] for i in
                 range(max(-d, 0), min(board_width, board_height - d))),
                winning_length)
            if winner != 0:
                return winner
    ...
    def play_game(self, plus_player_function, minus_player_function, hard, board_width=7, board_height=6,
                  winning_length=4, log=True):
